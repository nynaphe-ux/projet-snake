...
pomme.piege = Image.open("../assets/pomme_piege.png")
pomme.piege = ImageTK.PhotoImage(pomme.piege)
...
def computeNextFrame(numFrame,coordonnee, objet):
    ...
    for p in range(len(objet)):
        if coordonnee[0][0] == objet [p][0] and coordonnee[p][1] == objet [p][1]:
    ...
    if game_over : 
        # Fin de partie
        TEXTE = "GAME OVER"
        normal_font = tkfont.Font(family="Helvetica", size=24, weight="bold")
...
if __name__ == "__main__":
    ...
    vitesse = 110  # Vitesse de départ
    score = 0
    objet = []
    # Création de la pomme normale
    objet = []
    objet.append([randint(1, 24) * 20, randint(1, 24) * 20, 0])
    
    # Création du piège (la pomme piégée)
    pieges = []
    pieges.append([randint(1, 24) * 20, randint(1, 24) * 20, 0])
